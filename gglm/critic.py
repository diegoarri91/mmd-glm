...
class Critic:
    ...
    def fit(self, t, mask_spikes, u, r, y, newton_kwargs=None, verbose=False):

        newton_kwargs = {} if newton_kwargs is None else newton_kwargs
        
        objective_kwargs = self.objective_kwargs(t, mask_spikes, u, r, y)
...
#         self.u0 = theta[2
    ...
    def transform(self, t, mask_spikes, u, r):
        ...
        theta = self.get_params()
        wasserstein_kwargs = self.wasserstein_distance_kwargs(t, mask_spikes, u, r, None)
...
#             print(np.mean(a[y==1]))
    ...
    def wasserstein_distance_kwargs(self, t, mask_spikes, u, r, y):
...
#         r_spk = np.array([np.sum(r[mask_spikes[:, sw], sw]) for sw in range(mask_spikes.shape[1])]) if 'r_spk' in self.features else None
    ...
    def objective_kwargs(self, t, mask_spikes, u, r, y):
        return self.wasserstein_distance_kwargs(t=t, mask_spikes=mask_spikes, u=u, r=r, y=y)